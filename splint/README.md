...
| `pairing`       | a file with no test named after it, and a test with no file named after it                                                                                                                               | files, tests, paired and standalone, per package                                                                                                            |
...
`pairing` reads the two directions at different weights. `unpaired`, a file with no test beside it, is a WARN and is excused when every exported symbol of the file is referenced by a test elsewhere. `orphan`, a test with no file beside it, is an ERROR: `foo_test.go` names `foo.go`, and a directory holding only the first is a rename or a deletion that stopped halfway. A directory whose package name contains `test` is excused from `orphan`, because `fstest`, `httptest` and a repository's own `tests` are test code rather than tests of code; the `_test` suffix every external test scope carries comes off the name before it is read.
