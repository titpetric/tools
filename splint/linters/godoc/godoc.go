...
func (l *Linter) Lint(ctx context.Context, root *model.DocumentRoot) (model.LintReport, error) {
	...
	for _, def := range root.Packages {
		...
		for _, decls := range []model.DeclarationList{def.Types, def.Funcs, def.Consts, def.Vars} {
			results = append(results, check(def.Package, decls)...)
		}
	}
...
func check(pkg model.Package, decls model.DeclarationList) Results {
	var results Results
...
func validate(pkg model.Package, decl *model.Declaration) Results {
	doc := strings.TrimSpace(decl.Doc)
	if doc == "" {
		return Results{result(pkg, decl, RuleMissing, "exported symbol lacks a godoc comment")}
	}
	...
	if len(decl.Names) > 1 {
		if !endsInPunctuation(doc) {
			return Results{result(pkg, decl, RuleFormat, "godoc should end in punctuation")}
		}
		return nil
	}
	...
	if !strings.EqualFold(words[0], decl.Name) {
		return Results{result(pkg, decl, RuleFormat, fmt.Sprintf("godoc should open on %q and opens on %q", decl.Name, words[0]))}
	}
	if !endsInPunctuation(doc) {
		return Results{result(pkg, decl, RuleFormat, "godoc should end in punctuation")}
	}
	if lines := strings.Count(doc, "\n") + 1; lines > maxDocLines {
		return Results{result(pkg, decl, RuleVerbose, fmt.Sprintf("godoc runs to %d lines, which usually says the symbol does too much", lines))}
	}
